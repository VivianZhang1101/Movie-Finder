...
/**
 * The test method for front end developer
 * @author fangjunzhou
 * @version 1.0
 */
public class TestFrontend {
  ...
  public void runTests() {
    ...
    System.out.print("Test 'r' load rating selection screen (WARNING: if pressing 'x' twice from the rating selection screen does not exit app, test won't exit and run indefinitely!): ");
    if (this.testFrontendRForRatings()) {
      System.out.println("PASSED");
    } else {
      System.out.println("FAILED");
    }
    System.out.print("Test 'g' load rating selection screen and select Horror genre (WARNING: if pressing 'x' twice from the rating selection screen does not exit app, test won't exit and run indefinitely!): ");
    if (this.testFrontedGetMovie()){
      System.out.println("PASSED");
    } else {
      System.out.println("FAILED");
    }
   ...
   * the app by pressing 'x' to go back to the main mode and another 'x' to exit.
   * The test succeeds if the rating selectio screen contains all ten ratings
   * from the data. It fails if any of them are missing, the front end has not
   * been instantiated (is null), or there is an exception.
   * @return true if the test passed, false if it failed
   */
  ...
  /**
   * This test runs the front end and redirects its output to a string. It then
   * passes in 'g' as a command to go to the genre selection mode.
   * It then enter '1' to select Horror general. It then exists
   * the app by pressing 'x' to go back to the main mode and another 'x' to exit.
   * The test succeeds if return the correct movie. It fails if any of them are missing, the front end has not
   * been instantiated (is null), or there is an exception.
   * @return true if the test passed, false if it failed
   */
  public boolean testFrontedGetMovie(){
    PrintStream standardOut = System.out;
    InputStream standardIn = System.in;
    try {
      // set the input stream to our input (with an x to test of the program exists)
      ...
      System.setIn(standardIn);
      String appOutput = outputStreamCaptor.toString();
      if (frontend == null || !appOutput.contains("1)Comedy")) {
        // test failed
        return false;
      } else {
        // test passed
